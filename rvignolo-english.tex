...
\begin{center}\includegraphics[width=3.5cm]{photo-cv-1.jpg}\end{center}
\end{minipage}
...
% -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
\begin{secciondoscol}{Computer Skills}
 \filadosendos{GNU/Linux}{Advanced}
 \filadosendos{C}{Advanced}
 \filadosendos{\CC}{Intermediate}
 \filadosendos{Python}{Intermediate}
 \filadosendos{Fortran}{Intermediate}
 \filadosendos{Scripting}{Intermediate (including bash, AWK, sed, curl, etc)}
 \filadosendos{git and hg}{Advanced}
 \filadosendos{\LaTeX}{Advanced}
\end{secciondoscol}

% -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
\begin{secciontrescol}{Experience}

\filatresentres{\footnotesize 2017--current}{%
\azul{CRISIL}, Buenos Aires\\
\emph{Senior Quant Analyst}
}{\href{https://www.crisil.com}{\includegraphics[width=0.9cm]{logos/crisil}}}{-0.6cm}

\filadosentres{}{%
Scrutiny and testing of risk models.\\
Understand the model scope, investment products involved and methodology in detail.\\
Assure cutting-edge techniques for FO model testing design.
}
...
\begin{tabular}{rp{\anchoparrafo}}
 Project & MDDT Documentation Project\\
 Client  & Tier-1 US Bank\\
 Period  & 2017--current \\
\end{tabular}}%
\nopagebreak%

\filadosentres{}{Identify the key model assumptions and limitations in order to properly design testing cases using cutting-edge methodologies.}

\filadosentres{}{Scrutiny of assumption and limitation using quantitative and statistical tools.}

\filadosentres{}{Front Office testing design for derivative pricing models for vanilla and exotic derivatives, including numerical stability, consistency tests, arbitrage tests, pricing and Greek analysis, sensitivity tests, stress tests, CCAR/DFAST tests, Profit \& Loss tests, among others.}

\filadosentres{}{Independently develop methodologies to challenge the model by benchmarking it with market known or alternative methods.}

\filadosentres{}{Propose solution for the identified unpleasant behaviors or weaknesses, limitations or restrictions for FO models and provide relevant feedback to the Line of Business.}

\filadosentres{}{Produce written summaries of research, scrutiny and assessment of testing results in fulfillment with Model Development Document standards as per regulatory requirement.}
...
\filadosentres{}{Development of the \emph{mate} calculation code, implemented in C as a \emph{wasora} plugin, capable of simulating one-dimensional two-phase flow for different geometries.}

\filasep

\filatresentres{\footnotesize 2014--2017}{%
...
\software{mate}{Mate is a program that solves steady-state systems involving mixtures of steam and water in different geometries. It is intended to be free and open in the short term.}{https://bitbucket.org/besna/plugin-mate}
